...
function lowPopulationAnimals(data) {
    const endangered_animals = data.filter(d => d.population <= 5);
...
function USApop(data) {
    const popArray = [];
    data.forEach(d => popArray.push(d.population)); //create array to use with reduce()
    const totalPop = popArray.reduce((a, b) => a + b); //initial value is 0 by default, so does not need to be declared
    return totalPop
}
...
// ----> Question for review: Can you explain why it doesn't work when volume() is declared as an arrow function? <----
...
class CubeMaker extends CuboidMaker {
    // Take the dimensions of a cuboid and convert it to a cube of the same volume and surface area
    // The formulas for volume and surface area of a cube are identical and do not need to be repeated
    constructor(props) {
        super(props)
        this.width = (this.width + this.height + this.length) / 3;
        this.length = (this.width + this.height + this.length) / 3;
        this.height = (this.width + this.height + this.length) / 3;
    }
}
...
console.log(myCube.volume(), myCube.surfaceArea())
